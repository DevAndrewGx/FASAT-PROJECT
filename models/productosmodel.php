...
class ProductosModel extends Model implements JsonSerializable
{
    ...
    function getProductsByCategory($idCategoria)
    {
        ...
        try {
            ...
            $query  = $this->prepare('SELECT p.nombre, p.id_pinventario, p.precio, p.descripcion, c.nombre_categoria FROM productos_inventario p INNER JOIN categorias c ON  c.id_categoria = p.id_categoria WHERE c.id_categoria = :id');
